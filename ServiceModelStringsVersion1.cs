...
public class XmlDictionaryImpl : IXmlDictionary
{
    private readonly XmlDictionaryString[] dic;

    public IEnumerable<(int key, string value)> GetItems()
    {
        foreach (var item in dic.Where(d => d?.Value != null))
        {
            yield return (item.Key, item.Value);
        }
    }

    public XmlDictionaryImpl()
    {
        var wordTable = new ServiceModelStringsVersion1();
        dic = new XmlDictionaryString[wordTable.Count];
        for (int i = 0; i < wordTable.Count; i++)
        {
            dic[i] = new XmlDictionaryString(this, wordTable[i], i);
        }
    }

    public XmlDictionaryImpl(IEnumerable<(int key, string value)> words)
    {
        var wordsList = words.ToList();
        if (wordsList.Select(w => w.key).Distinct().Count() == wordsList.Count && wordsList.Select(w => w.value).Distinct().Count() == wordsList.Count)
        {
            var max = wordsList.Max(i => i.key);
            dic = new XmlDictionaryString[max + 1];
            foreach (var (key, value) in wordsList)
            {
                dic[key] = new XmlDictionaryString(this, value, key);
            }
        }
        else
        {
    ...
    public bool TryLookup(string key, out XmlDictionaryString value)
    {
        ...
        value = dic.FirstOrDefault(ds => ds.Value == key);
        return value != null;
    }
    ...
    public bool TryLookup(XmlDictionaryString key, out XmlDictionaryString value)
    {
        ...
        value = dic.FirstOrDefault(ds => ds.Value == key.Value && ds.Key == key.Key);
        return value != null;
    }
}

public class ServiceModelStringsVersion1
{
    ...
    public  int Count { get { return 487; } }
